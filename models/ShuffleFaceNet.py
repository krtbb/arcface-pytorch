...
# copied from https://raw.githubusercontent.com/mbfaria/ShuffleFaceNet_Pytorch/master/core/model.py
...
class ShuffleFaceNet(nn.Module):
    def __init__(self, outsize, stages_repeats=[4, 8, 4], stages_out_channels=[24, 176, 352, 704, 1024], inverted_residual=InvertedResidual):
    ...
    def _forward_impl(self, x):
        ...
        x = self.linearconv(x)
        x = x.view(x.size(0), self.outsize, 1, 1)
        x = self.bn(x)
        x = x.view(x.size(0), -1)
